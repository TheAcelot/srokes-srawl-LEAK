...
class BattleEndMessage(PiranhaMessage):
    ...
    def encode(self, fields, player):
        ...
        playerData = db_instance.getPlayer([player.ID[0], player.ID[1]])
        ...
        for heroEntry in fields["Heroes"]:
            ...
            self.writeVInt(1)
            for i in range(1):
                self.writeVInt(playerData["OwnedBrawlers"][f"{player.SelectedBrawler}"]["Trophies"] - 1250) #trophies
            self.writeVInt(1)
            for i in range(1):
                self.writeVInt(11) #lvl
            self.writeVInt(1)
            for i in range(1):
                self.writeVInt(0)
            self.writeVInt(0)
        ...
        self.writeVInt(1)
        self.writeVInt(playerData["OwnedBrawlers"][f"{player.SelectedBrawler}"]["Trophies"] - 1250)
        self.writeVInt(playerData["OwnedBrawlers"][f"{player.SelectedBrawler}"]["HighestTrophies"] - 1250)
